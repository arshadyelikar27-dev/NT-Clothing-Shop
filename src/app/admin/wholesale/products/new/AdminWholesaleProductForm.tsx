...
export function AdminWholesaleProductForm({ categories }: { categories: Category[] }) {
  const router = useRouter();
  
  const [name, setName] = useState("");
  const [price, setPrice] = useState(""); // base retail price
  const [categoryId, setCategoryId] = useState(categories[0]?.id || "");
  const [description, setDescription] = useState("");
  const [videoUrl, setVideoUrl] = useState("");
  
  // Wholesale Tiers
  const [wholesaleTiers, setWholesaleTiers] = useState<{ minQty: number; price: number }[]>([]);

  const addTier = () => setWholesaleTiers([...wholesaleTiers, { minQty: 10, price: 0 }]);
  ...
  const removeTier = (index: number) => setWholesaleTiers(wholesaleTiers.filter((_, i) => i !== index));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (wholesaleTiers.length === 0) {
      setError("At least one wholesale pricing tier is required for a wholesale product");
      return;
    }
    ...
    <form onSubmit={handleSubmit} style={{ backgroundColor: "white", border: "1px solid #E4DDD3", padding: "32px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        <div style={{ paddingBottom: "24px", borderBottom: "1px solid #F3EFEA" }}>
          <h3 style={{ fontSize: "16px", fontWeight: 600, color: "#1A1918", marginBottom: "16px" }}>Wholesale Product Details</h3>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <div>
              ...
              <input
                type="text"
                required
                placeholder="e.g. Pure Cambric Cotton Fabric (Wholesale Set)"
                value={name}
        ...
        <div style={{ paddingBottom: "24px", borderBottom: "1px solid #F3EFEA" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
            <h3 style={{ fontSize: "16px", fontWeight: 600, color: "#1A1918" }}>Wholesale Pricing Tiers *</h3>
          ...
          {wholesaleTiers.length === 0 ? (
            <div style={{ padding: "20px", textAlign: "center", backgroundColor: "#FAF7F2", border: "1px dashed #E4DDD3", fontSize: "13px", color: "#8A8279" }}>
              Click "+ Add Tier" to define wholesale pricing rules.
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              {wholesaleTiers.map((tier, idx) => (
                <div key={idx} style={{ display: "flex", gap: "16px", alignItems: "flex-end", backgroundColor: "#FAF7F2", padding: "16px", border: "1px solid #E4DDD3" }}>
                  <div style={{ flex: 1 }}>
                    <label style={{ display: "block", fontSize: "12px", fontWeight: 600, marginBottom: "4px" }}>Minimum Quantity</label>
                    <input type="number" min={1} required value={tier.minQty || ""} onChange={(e) => updateTier(idx, "minQty", parseInt(e.target.value) || 0)} className="input" placeholder="e.g. 10" />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={{ display: "block", fontSize: "12px", fontWeight: 600, marginBottom: "4px" }}>Wholesale Price (₹)</label>
                    <input type="number" min={0} required value={tier.price || ""} onChange={(e) => updateTier(idx, "price", parseFloat(e.target.value) || 0)} className="input" placeholder="e.g. 250" />
                  </div>
                  <button type="button" onClick={() => removeTier(idx)} style={{ padding: "10px", backgroundColor: "#FEE2E2", color: "#991B1B", border: "1px solid #FCA5A5", borderRadius: "4px", fontSize: "12px", fontWeight: 600 }}>Remove</button>
                </div>
              ))}
            </div>
          )}
        ...
        <div style={{ marginTop: "16px", paddingTop: "24px", borderTop: "1px solid #F3EFEA" }}>
          <button type="submit" disabled={isSubmitting} className="btn btn-primary" style={{ padding: "14px 32px" }}>
            {isSubmitting ? "Uploading & Saving..." : "Save & Publish Wholesale Product"}
          </button>
        </div>
      </div>
    </form>
